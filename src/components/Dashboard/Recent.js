...
const Recent = ({symbol, conversionRate}) => {
    ...
    return (
        ...
        <div className={styles.recent}
             style={{height: windowSize.innerWidth < 500 ? `${windowSize.innerHeight - 465}px` : windowSize.innerWidth < 1000 ? `${windowSize.innerHeight - 560}px` : `${windowSize.innerHeight - 560}px`}}>
            {showErrorModal && <ErrorModal showModal={showErrorModal} setShowModal={setShowErrorModal}
                                           h={"Functionality not available yet!"}
                                           p={"The filter by date feature is not yet available, please come back later"}/>}
            <div className={styles.topBar}>
                <h3>Recent transactions</h3>
                <button className={styles.date_pick} onClick={() => {
                    setShowErrorModal(true)
                }}><CalendarBlank size={16}/> <span>Select date</span></button>
            </div>

            <div className={styles.switchcont}>
                <button className={`${styles.button1} ${mode === "all" ? styles.activebtn : ""}`}
                        onClick={() => buttonClickHandler("all")}>View all
                </button>
                <button className={`${styles.button2} ${mode === "transfers" ? styles.activebtn : ""}`}
                        onClick={() => buttonClickHandler("transfers")}>Transfers
                </button>
                <button className={`${styles.button3} ${mode === "cards" ? styles.activebtn : ""}`}
                        onClick={() => buttonClickHandler("cards")}>Cards
                </button>
            </div>
                ...
                <table>
                    ...
                    {mode === "all" && recentAll.map((item, index) => {
                        ...
                        return (
                            item.type === "debit" ? (
                                <tr key={index}>
                                    <td className={styles.merchcont}><img src={merchantLogos[item.merchant]}
                                                                          className={styles.logo}
                                                                          alt={"logo"}/>{item.merchant}</td>
                                    <td className={styles.debit}>{`- ${symbol}${amt}`}</td>
                                    <td>{formatDate(item.datetime)}</td>
                                    <td className={styles.mode}><img className={styles.card}
                                                                     src={modesImg[item.exactMode.split(" ")[0]]}
                                                                     alt={"logo"}/>
                                        <div className={styles.cardDet}><span>{item.exactMode}</span><span>Expiry 06/2024</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                <tr key={index}>
                                    <td className={styles.merchcont}><img src={modesImg[item.sender]}
                                                                          className={styles.logo}
                                                                          alt={"logo"}/>{item.sender}</td>
                                    <td className={styles.credit}>{`+ ${symbol}${amt}`}</td>
                                    <td>{formatDate(item.datetime)}</td>
                                    <td className={styles.mode}><img className={styles.card} src={modesImg[item.sender]}
                                                                     alt={"logo"}/> {item.sender}</td>
